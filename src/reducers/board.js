import { generateGrid, isFieldClickable } from "../utils/boardFunctions";
...
function generateLevel(fields, { x, y, level }) {
  const currentField = fields.find(item => item.x === x && item.y === y);
...
function generateFields(currentField, fields, level) {
  const availableFields = fields.filter(
    item =>
      !["clickable", "clicked"].includes(item.status) &&
      isFieldClickable(item, {
        x: currentField.x,
        y: currentField.y
      })
  );
  ...
  return level ? generateFields(nextField, fields, --level) : fields;
}
